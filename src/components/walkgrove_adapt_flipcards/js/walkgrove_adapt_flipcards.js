  ...
  var FlipcardsView = ComponentView.extend({
    ...
    postRender: function() {
      ...
      this.model.get('_items').forEach((item, i) => {
        ...
        if (this.model.get('_fullImage') === true) {
          this.$('.flipcards__widget').eq(i).addClass('full-image');
        }

        this.$('.flipcards__widget').eq(i).css({
          minHeight: this.model.get('_minHeight') ? this.model.get('_minHeight'): 400,
          maxHeight: this.model.get('_minHeight') ? this.model.get('_minHeight'): 450
        });
