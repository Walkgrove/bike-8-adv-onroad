define([
  ...
  var Bookmarking = Backbone.Controller.extend({
    ...
    currentOnscreenLocation: null,
    prevOnscreenLocation: null,
    ...
    checkLocation: function() {
      ...
      for (var i = 0, l = this.watchViews.length; i < l; i++) {
        ...
        if (measurements.percentInview > highestOnscreen) {
          ...
          highestOnScreenComponent = view.model;
          componentNum = i;

          if(highestOnScreenComponent.get('_component') === "reflection") {
            Adapt.trigger('reflect:data');
          }
        }
      }

      this.currentOnscreenLocation = highestOnscreenLocation;
      //console.log('prevOnscreenLocation: ' + this.prevOnscreenLocation, 'currentOnscreenLocation: ' + this.currentOnscreenLocation);
        if((this.prevOnscreenLocation !== this.currentOnscreenLocation) || componentNum === 0) {
            this.prevOnscreenLocation = this.currentOnscreenLocation;
       
            if(highestOnScreenComponent && Adapt.config.get('_sound')._isActive === true) {
              //set audiuo based on inview component...
              if(highestOnScreenComponent.get('_audio') && (highestOnScreenComponent.get('_audio')._src !== this.currentAudioSrc)) {

                if((Adapt.offlineStorage.get('location') !== highestOnscreenLocation)) {
                  Adapt.trigger('audio:stop');
                }
                
                if (highestOnScreenComponent.get('_audio') ) {
                  if((highestOnScreenComponent.get('_component') === "text" && highestOnscreen > 60)) {
                    this.currentAudioSrc = highestOnScreenComponent.get('_audio')._src;
                    Adapt.trigger('audio:setup', {src: highestOnScreenComponent.get('_audio')._src, transcript: highestOnScreenComponent.get('_audio').transcript});
                  }
                  if(highestOnScreenComponent.get('_component') !== "text") {
                    this.currentAudioSrc = highestOnScreenComponent.get('_audio')._src;
                    Adapt.trigger('audio:setup', {src: highestOnScreenComponent.get('_audio')._src, transcript: highestOnScreenComponent.get('_audio').transcript});
                  }
                } else {
      ...
      if (highestOnscreenLocation) {
         // set location as most inview component
         this.setLocationID(highestOnscreenLocation);
      }
