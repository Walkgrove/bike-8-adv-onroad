define([
  ...
  var NarrativeView = ComponentView.extend({
    ...
    setStage: function(item) {
      ...
      this.$('.narrative__slider-image-container').removeClass('is-active');
      $slideGraphics.filter('[data-index="'+index+'"]').addClass('is-active');
    ...
    replaceTitle: function() {
      if (Adapt.device.screenSize !== 'small') {
        this.$('.component__title-inner').html(this.model.get('title'));
      } else if (this.model.get('mobileTitle')) {
        this.$('.component__title-inner').html(this.model.get('mobileTitle'));
      }
    }
